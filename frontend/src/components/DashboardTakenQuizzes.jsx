...
const DashboardTakenQuizzes = ({ loading, api }) => {
    // Variables
    const sortOptions = [
        {
            value: 'last_taken',
    ...
    const fetchQuizzes = async (selectedYears, selectedCategories, selectedSort) => {
        ...
        try {
            const response = await api.get(`dashboard/taken-quizzes/${page}?sort=${selectedSort}${yearsCategariesQuery}`);
            setQuizzes(response.data.user_quiz_scores.quizzes);
            setQuizzesTaken(response.data.user_quiz_scores.quizzes_taken);
            setTotalAttempts(response.data.user_quiz_scores.total_attempts);
    ...
    return (
        <>
            <section className="user-dashboard__stats">
                <p>Quizzes taken: <span>{ quizzesTaken ?? 0 }</span></p>
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="user-dashboard__tiles-container__tile__info">
                                    ...
                                    <hr className='divider--no-margin' />
                                    <div className="user-dashboard__tiles-container__tile__info__buttons">
                                        <Link to={`/quiz/${quiz.id}`}>
                                            <button className="button--large--emerald-dark user-dashboard__tiles-container__tile__info__button">Take Quiz Again</button>
                                        </Link>
                                    </div>
                                </div>
                            </section>
...
export default DashboardTakenQuizzes;
